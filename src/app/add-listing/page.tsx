...
import {
  ...
  Warehouse, Key, Upload, X, Plus, Tag
} from "lucide-react"
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
        ...
        <div className="max-w-2xl mx-auto px-4 py-20 text-center">
          ...
          <Button onClick={() => router.push("/login")} className="bg-gradient-to-r from-orange-400 to-amber-500">
            Kirish
          </Button>
        </div>
        <Footer />
        ...
        <Card className="bg-white/80 backdrop-blur-xl border border-white/70 shadow-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 max-w-md">
                  {DEAL_TYPES.map((d) => (
                    <button
                      key={d.id}
                      onClick={() => handleChange("deal", d.id)}
                ...
                <>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="h-12 bg-white/90 border-2 border-white/70 rounded-xl"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-4 py-3 bg-white/90 border-2 border-white/70 rounded-xl text-gray-900 focus:border-orange-400 focus:ring-4 focus:ring-orange-400/20 outline-none resize-none"
                    />
                  </div>
                </>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) => { handleChange("region", e.target.value); handleChange("district", "") }}
                    className="w-full h-12 px-4 bg-white/90 border-2 border-white/70 rounded-xl text-gray-900 focus:border-orange-400 outline-none"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full h-12 px-4 bg-white/90 border-2 border-white/70 rounded-xl text-gray-900 focus:border-orange-400 outline-none disabled:opacity-50"
                  >
                    <option value="">Tanlang</option>
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              {isBuildingType && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <Label className="font-semibold">Xonalar soni</Label>
                    <Input type="number" placeholder="3" value={formData.rooms} onChange={(e) => handleChange("rooms", e.target.value)} className="h-12 bg-white/90 border-2 border-white/70 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <Label className="font-semibold">Maydon (m2)</Label>
                    <Input type="number" placeholder="78" value={formData.area} onChange={(e) => handleChange("area", e.target.value)} className="h-12 bg-white/90 border-2 border-white/70 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <Label className="font-semibold">Qavat</Label>
                    <Input type="number" placeholder="5" value={formData.floor} onChange={(e) => handleChange("floor", e.target.value)} className="h-12 bg-white/90 border-2 border-white/70 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <Label className="font-semibold">Jami qavatlar</Label>
                    <Input type="number" placeholder="9" value={formData.totalFloors} onChange={(e) => handleChange("totalFloors", e.target.value)} className="h-12 bg-white/90 border-2 border-white/70 rounded-xl" />
                  </div>
                </div>
              )}
              ...
              {formData.category === "LAND" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <Label className="font-semibold">Maydon (sotix)</Label>
                    <Input type="number" placeholder="6" value={formData.landArea} onChange={(e) => handleChange("landArea", e.target.value)} className="h-12 bg-white/90 border-2 border-white/70 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    ...
                    <select value={formData.landStatus} onChange={(e) => handleChange("landStatus", e.target.value)} className="w-full h-12 px-4 bg-white/90 border-2 border-white/70 rounded-xl">
                      <option value="">Tanlang</option>
                      <option value="residential">Turar joy uchun</option>
              ...
              {formData.category === "WAREHOUSE" && (
                <div className="space-y-2">
                  <Label className="font-semibold">Maydon (m2)</Label>
                  <Input type="number" placeholder="500" value={formData.area} onChange={(e) => handleChange("area", e.target.value)} className="h-12 bg-white/90 border-2 border-white/70 rounded-xl" />
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 max-w-md">
                <Label className="font-semibold">
                  {formData.deal === "RENT" ? "Oylik narx ($)" : "Narx ($)"} *
                </Label>
          ...
          {step === 5 && (
            <div className="space-y-4">
              ...
              <div className="border-2 border-dashed border-orange-300 rounded-xl p-8 text-center bg-orange-50/50">
                <Upload className="h-10 w-10 text-orange-500 mx-auto mb-3" />
                <p className="text-sm text-gray-600">Rasm yuklash tez orada qo'shiladi (Uploadthing)</p>
              </div>
            </div>
          )}
          ...
          {step === 6 && (
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-5 border border-orange-200 space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-gray-600">Joylashuv:</span><span className="font-semibold">{[formData.region, formData.district].filter(Boolean).join(", ") || "-"}</span></div>
              </div>
              <div className="p-3 bg-green-50 rounded-xl border border-green-200 text-sm text-green-700">
                E'lon moderatsiyadan o'tgach saytda ko'rinadi (odatda 24 soat ichida).
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">
            ...
            {step < 6 ? (
              <Button
                onClick={nextStep}
                disabled={(step === 1 && (!formData.category || !formData.deal || !formData.title))}
                className="bg-gradient-to-r from-orange-400 to-amber-500 hover:from-orange-500 hover:to-amber-600 text-white font-semibold"
              >
                Keyingi
